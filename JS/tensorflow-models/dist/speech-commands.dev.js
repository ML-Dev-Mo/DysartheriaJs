...
(function (global, factory) {
  (typeof exports === "undefined" ? "undefined" : _typeof(exports)) === 'object' && typeof module !== 'undefined' ? factory(exports, require('@tensorflow/tfjs-core'), require('util'), require('@tensorflow/tfjs-data'), require('@tensorflow/tfjs-layers')) : typeof define === 'function' && define.amd ? define(['exports', '@tensorflow/tfjs-core', 'util', '@tensorflow/tfjs-data', '@tensorflow/tfjs-layers'], factory) : factory(global.speechCommands = {}, global.tf, null, global.tf.data, global.tf);
})(void 0, function (exports, tf, util, tfd, tfl) {
  'use strict';
  /*! *****************************************************************************
  ...
  ***************************************************************************** */
      ...
      return __generator(this, function (_c) {
        switch (_c.label) {
          case 0:
            ...
            HTTPS_SCHEME = 'https://';
            FILE_SCHEME = 'file://';
            if (!(url.indexOf(HTTP_SCHEME) === 0 || url.indexOf(HTTPS_SCHEME) === 0)) return [3
            /*break*/
            , 3];
            return [4
            /*yield*/
          ...
          case 3:
            if (!(url.indexOf(FILE_SCHEME) === 0)) return [3
            /*break*/
            , 5];
            fs = require('fs');
            ...
            return [4
            /*yield*/
            , readFile(url.slice(FILE_SCHEME.length), {
              encoding: 'utf-8'
            })];
          ...
          case 5:
            throw new Error("Unsupported URL scheme in metadata URL: " + url + ". " + "Supported schemes are: http://, https://, and " + "(node.js-only) file://");
        }
      });
    });
  }
  ...
  function normalize(x) {
    ...
    return tf.tidy(function () {
      var _a = tf.moments(x),
          mean = _a.mean,
          variance = _a.variance; // Add an EPSILON to the denominator to prevent division-by-zero.
  ...
  function normalizeFloat32Array(x) {
    ...
    return tf.tidy(function () {
      var _a = tf.moments(tf.tensor1d(x)),
          mean = _a.mean,
  ...
  var BrowserFftFeatureExtractor =
  /** @class */
  function () {
    /**
    ...
    function BrowserFftFeatureExtractor(config) {
      ...
      tf.util.assert(this.overlapFactor >= 0 && this.overlapFactor < 1, function () {
        return "Expected overlapFactor to be >= 0 and < 1, " + ("but got " + _this.overlapFactor);
      });
        ...
        return __generator(this, function (_b) {
          switch (_b.label) {
            case 0:
              ...
              return [4
              /*yield*/
              , getAudioMediaStream(audioTrackConstraints)];
        ...
        return __generator(this, function (_a) {
          switch (_a.label) {
            case 0:
              ...
              return [4
              /*yield*/
              , this.spectrogramCallback(freqDataTensor, timeDataTensor)];
  ...
  function () {
    ...
    function Tracker(period, suppressionPeriod) {
      ...
      this.counter = 0;
      tf.util.assert(this.period > 0, function () {
        return "Expected period to be positive, but got " + _this.period;
      });
    }
    /**
     * Mark a frame.
  ...
  function balancedTrainValSplit(xs, ys, valSplit) {
    tf.util.assert(valSplit > 0 && valSplit < 1, function () {
      return "validationSplit is expected to be >0 and <1, " + ("but got " + valSplit);
    });
    return tf.tidy(function () {
      ...
      indicesByClasses.map(function (classIndices) {
        return tf.util.shuffle(classIndices);
      });
  ...
  function () {
    ...
    Dataset.prototype.addExample = function (example) {
      tf.util.assert(example != null, function () {
        return 'Got null or undefined example';
      });
      tf.util.assert(example.label != null && example.label.length > 0, function () {
        return "Expected label to be a non-empty string, " + ("but got " + JSON.stringify(example.label));
      });
      var uid = getUID();
    ...
    Dataset.prototype.merge = function (dataset) {
      ...
      tf.util.assert(dataset !== this, function () {
    ...
    Dataset.prototype.getExamples = function (label) {
      ...
      tf.util.assert(label in this.label2Ids, function () {
        return "No example of label \"" + label + "\" exists in dataset";
      });
      var output = [];
      this.label2Ids[label].forEach(function (id) {
        output.push({
          uid: id,
          example: _this.examples[id]
        });
      });
      return output;
    };
    ...
    Dataset.prototype.getData = function (label, config) {
      ...
      if (label != null) {
        tf.util.assert(vocab.indexOf(label) !== -1, function () {
          return "Label " + label + " is not in the vocabulary " + ("(" + JSON.stringify(vocab) + ")");
        });
      } else {
        // If all words are requested, there must be at least two words in the
        // vocabulary to make one-hot encoding possible.
        tf.util.assert(vocab.length > 1, function () {
          return "One-hot encoding of labels requires the vocabulary to have " + ("at least two words, but it has only " + vocab.length + " word.");
        });
      }
      ...
      if (sortedUniqueNumFrames.length === 1) {
        ...
        tf.util.assert(numFrames != null && Number.isInteger(numFrames) && numFrames > 0, function () {
          return "There are " + sortedUniqueNumFrames.length + " unique lengths among " + ("the " + _this.size() + " examples of this Dataset, hence numFrames ") + "is required. But it is not provided.";
        });
        tf.util.assert(numFrames <= sortedUniqueNumFrames[0], function () {
          return "numFrames (" + numFrames + ") exceeds the minimum numFrames " + ("(" + sortedUniqueNumFrames[0] + ") among the examples of ") + "the Dataset.";
        });
        hopFrames = config.hopFrames;
        tf.util.assert(hopFrames != null && Number.isInteger(hopFrames) && hopFrames > 0, function () {
          return "There are " + sortedUniqueNumFrames.length + " unique lengths among " + ("the " + _this.size() + " examples of this Dataset, hence hopFrames ") + "is required. But it is not provided.";
        });
      ...
      return tf.tidy(function () {
        ...
        for (var i = 0; i < vocab.length; ++i) {
          ...
          var _loop_1 = function _loop_1(id) {
            ...
            if (uniqueFrameSize == null) {
              ...
              tf.util.assert(frameSize === uniqueFrameSize, function () {
                return "Mismatch in frameSize  " + ("(" + frameSize + " vs " + uniqueFrameSize + ")");
              });
            }
            ...
            var _loop_2 = function _loop_2(window_1) {
        ...
        if (config.getDataset) {
          ...
          var valSplit_1 = config.datasetValidationSplit == null ? 0.15 : config.datasetValidationSplit;
          tf.util.assert(valSplit_1 > 0 && valSplit_1 < 1, function () {
            return "Invalid dataset validation split: " + valSplit_1;
          });
          ...
          var _b = balancedTrainValSplitNumArrays(xArrays, yArrays, valSplit_1),
              trainXs = _b.trainXs,
              trainYs = _b.trainYs,
              valXs = _b.valXs,
              valYs = _b.valYs; // TODO(cais): The typing around Float32Array is not working properly
          // for tf.data currently. Tighten the types when the tf.data bug is
          // fixed.
          // tslint:disable:no-any


          var xTrain = tfd.array(trainXs).map(function (x) {
            return tf.tensor3d(x, [numFrames, uniqueFrameSize, 1]);
          });
          var yTrain = tfd.array(trainYs).map(function (y) {
            return tf.squeeze(tf.oneHot([y], vocab.length), [0]);
          }); // TODO(cais): See if we can tighten the typing.

          var trainDataset = tfd.zip({
            xs: xTrain,
            ys: yTrain
          });
          ...
          var xVal = tfd.array(valXs).map(function (x) {
            return tf.tensor3d(x, [numFrames, uniqueFrameSize, 1]);
          });
          var yVal = tfd.array(valYs).map(function (y) {
            return tf.squeeze(tf.oneHot([y], vocab.length), [0]);
          });
          var valDataset = tfd.zip({
            xs: xVal,
            ys: yVal
          });
          ...
          if (shuffle) {
            ...
            xTensors = zipped_1.map(function (item) {
              return item.x;
            });
            labelIndices = zipped_1.map(function (item) {
              return item.y;
            });
          }

          var targets = label == null ? tf.cast(tf.oneHot(tf.tensor1d(labelIndices, 'int32'), vocab.length), 'float32') : undefined;
          return {
    ...
    Dataset.prototype.augmentByMixingNoise = function (xs, labelIndices, ratio) {
      ...
      if (noiseExampleIndices.length === 0) {
        throw new Error("Cannot perform augmentation by mixing with noise when " + ("there is no example with label " + BACKGROUND_NOISE_TAG));
      }
      ...
      var _loop_3 = function _loop_3(index) {
        ...
        var signalTensor = isTypedArray ? tf.tensor1d(xs[index]) : xs[index];
        var noiseTensor = isTypedArray ? tf.tensor1d(xs[noiseIndex]) : xs[noiseIndex];
        var mixed = tf.tidy(function () {
          return normalize(tf.add(signalTensor, tf.mul(noiseTensor, ratio)));
        });
      ...
      console.log("Data augmentation: mixing noise: added " + mixedXTensors.length + " " + "examples");
    ...
    Dataset.prototype.setExampleKeyFrameIndex = function (uid, keyFrameIndex) {
      ...
      var numFrames = spectrogram.data.length / spectrogram.frameSize;
      tf.util.assert(keyFrameIndex >= 0 && keyFrameIndex < numFrames && Number.isInteger(keyFrameIndex), function () {
        return "Invalid keyFrameIndex: " + keyFrameIndex + ". " + ("Must be >= 0, < " + numFrames + ", and an integer.");
      });
    ...
    Dataset.prototype.serialize = function (wordLabels) {
      ...
      var vocab = this.getVocabulary();
      tf.util.assert(!this.empty(), function () {
      ...
      if (wordLabels != null) {
        ...
        wordLabels.forEach(function (wordLabel) {
          if (vocab.indexOf(wordLabel) === -1) {
            throw new Error("Word label \"" + wordLabel + "\" does not exist in the " + "vocabulary of this dataset. The vocabulary is: " + (JSON.stringify(vocab) + "."));
          }
        });
      }
  ...
  function arrayBuffer2SerializedExamples(buffer) {
    tf.util.assert(buffer != null, function () {
    ...
    tf.util.assert(descriptor === DATASET_SERIALIZATION_DESCRIPTOR, function () {
  ...
  function getValidWindows(snippetLength, focusIndex, windowLength, windowHop) {
    ...
    tf.util.assert(Number.isInteger(windowLength) && windowLength > 0, function () {
      return "windowLength must be a positive integer, but got " + windowLength;
    });
    tf.util.assert(Number.isInteger(windowHop) && windowHop > 0, function () {
      return "windowHop must be a positive integer, but got " + windowHop;
    });
    tf.util.assert(windowLength <= snippetLength, function () {
      return "windowLength (" + windowLength + ") exceeds snippetLength " + ("(" + snippetLength + ")");
    });
    tf.util.assert(focusIndex < snippetLength, function () {
      return "focusIndex (" + focusIndex + ") equals or exceeds snippetLength " + ("(" + snippetLength + ")");
    });
  ...
  function getMaxIntensityFrameIndex(spectrogram) {
    return tf.tidy(function () {
      return tf.argMax(spectrogram2IntensityCurve(spectrogram));
    });
  }
  ...
  var BrowserFftSpeechCommandRecognizer =
  /** @class */
  function () {
    /**
    ...
    function BrowserFftSpeechCommandRecognizer(vocabulary, modelArtifactsOrURL, metadataOrURL) {
      ...
      tf.util.assert(modelArtifactsOrURL == null && metadataOrURL == null || modelArtifactsOrURL != null && metadataOrURL != null, function () {
      ...
      if (modelArtifactsOrURL == null) {
        if (vocabulary == null) {
          ...
          tf.util.assert(BrowserFftSpeechCommandRecognizer.VALID_VOCABULARY_NAMES.indexOf(vocabulary) !== -1, function () {
            return "Invalid vocabulary name: '" + vocabulary + "'";
          });
        }

        this.vocabulary = vocabulary;
        this.modelArtifactsOrURL = this.MODEL_URL_PREFIX + "/" + this.vocabulary + "/model.json";
        this.metadataOrURL = this.MODEL_URL_PREFIX + "/" + this.vocabulary + "/metadata.json";
      } else {
        tf.util.assert(vocabulary == null, function () {
        ...
        return __generator(this, function (_a) {
          switch (_a.label) {
            ...
            case 1:
              ...
              tf.util.assert(probabilityThreshold >= 0 && probabilityThreshold <= 1, function () {
                return "Invalid probabilityThreshold value: " + probabilityThreshold;
              });
              ...
              if (config.suppressionTimeMillis < 0) {
                throw new Error("suppressionTimeMillis is expected to be >= 0, " + ("but got " + config.suppressionTimeMillis));
              }

              overlapFactor = config.overlapFactor == null ? 0.5 : config.overlapFactor;
              tf.util.assert(overlapFactor >= 0 && overlapFactor < 1, function () {
                return "Expected overlapFactor to be >= 0 and < 1, but got " + overlapFactor;
              });
                  ...
                  return __generator(this, function (_c) {
                    switch (_c.label) {
                      case 0:
                        ...
                        if (!config.includeEmbedding) return [3
                        /*break*/
                        , 2];
                        return [4
                        /*yield*/
                      ...
                      case 5:
                        ...
                        if (!(maxScore < probabilityThreshold)) return [3
                        /*break*/
                        , 6];
                        return [2
                        /*return*/
                      ...
                      case 6:
                        ...
                        if (!config.includeSpectrogram) return [3
                        /*break*/
                        , 8];
                        _a = {};
                        return [4
                      ...
                      case 7:
                        spectrogram = (_a.data = _c.sent(), _a.frameSize = this.nonBatchInputShape[1], _a);
                        _c.label = 8;
                      ...
                      case 8:
                        ...
                        if (!invokeCallbackOnNoiseAndUnknown) {
                          ...
                          if (this.words[maxIndex] === BACKGROUND_NOISE_TAG || this.words[maxIndex] === UNKNOWN_TAG) {
                            wordDetected = false;
                          }
                        }
              ...
              return [4
              /*yield*/
              , this.audioDataExtractor.start(config.audioTrackConstraints)];
        ...
        return __generator(this, function (_a) {
          switch (_a.label) {
            ...
            case 1:
              ...
              if (!(typeof this.modelArtifactsOrURL === 'string')) return [3
              /*break*/
              , 3];
              return [4
              /*yield*/
              , tfl.loadLayersModel(this.modelArtifactsOrURL)];
            ...
            case 3:
              return [4
              /*yield*/
              , tfl.loadLayersModel(tf.io.fromMemory(this.modelArtifactsOrURL.modelTopology, this.modelArtifactsOrURL.weightSpecs, this.modelArtifactsOrURL.weightData))];
            ...
            case 5:
              ...
              if (model.inputs.length !== 1) {
                throw new Error("Expected model to have 1 input, but got a model with " + (model.inputs.length + " inputs"));
              }

              if (model.inputs[0].shape.length !== 4) {
                throw new Error("Expected model to have an input shape of rank 4, " + ("but got an input shape of rank " + model.inputs[0].shape.length));
              }

              if (model.inputs[0].shape[3] !== 1) {
                throw new Error("Expected model to have an input shape with 1 as the last " + "dimension, but got input shape" + (JSON.stringify(model.inputs[0].shape[3]) + "}"));
              }

              outputShape = model.outputShape;

              if (outputShape.length !== 2) {
                throw new Error("Expected loaded model to have an output shape of rank 2," + ("but received shape " + JSON.stringify(outputShape)));
              }

              if (outputShape[1] !== this.words.length) {
                throw new Error("Mismatch between the last dimension of model's output shape " + ("(" + outputShape[1] + ") and number of words ") + ("(" + this.words.length + ")."));
              }
              ...
              model.inputs[0].shape.slice(1).forEach(function (dimSize) {
                return _this.elementsPerExample *= dimSize;
              });
              this.warmUpModel();
        ...
        return __generator(this, function (_b) {
          switch (_b.label) {
            case 0:
              ...
              if (!(typeof this.metadataOrURL === 'string')) return [3
              /*break*/
              , 2];
              return [4
              /*yield*/
              , loadMetadataJson(this.metadataOrURL)];
      ...
      return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_f) {
          switch (_f.label) {
            ...
            case 3:
              if (input instanceof tf.Tensor) {
                ...
                if (input.length % this.elementsPerExample) {
                  throw new Error("The length of the input Float32Array " + input.length + " " + "is not divisible by the number of tensor elements per " + ("per example expected by the model " + this.elementsPerExample + "."));
                }

                numExamples = input.length / this.elementsPerExample;
                inputTensor = tf.tensor4d(input, [numExamples].concat(this.nonBatchInputShape));
              }
              ...
              if (!config.includeEmbedding) return [3
              /*break*/
              , 5]; // Optional inclusion of embedding (internal activation).
            ...
            case 13:
              _c.spectrogram = (_d.data = _e, _d.frameSize = this.nonBatchInputShape[1], _d);
              _f.label = 14;
              ...
              return __awaiter(_this, void 0, void 0, function () {
                ...
                return __generator(this, function (_c) {
                  switch (_c.label) {
                    ...
                    case 2:
                      _a.apply(void 0, [(_b.data = _c.sent(), _b.frameSize = this.nonBatchInputShape[1], _b)]);
    ...
    BrowserFftSpeechCommandRecognizer.prototype.createTransfer = function (name) {
      ...
      tf.util.assert(name != null && typeof name === 'string' && name.length > 1, function () {
        return "Expected the name for a transfer-learning recognized to be a " + ("non-empty string, but got " + JSON.stringify(name));
      });
      tf.util.assert(this.transferRecognizers[name] == null, function () {
        return "There is already a transfer-learning model named '" + name + "'";
      });
    ...
    BrowserFftSpeechCommandRecognizer.prototype.checkInputTensorShape = function (input) {
      ...
      if (input.shape.length !== expectedRank) {
        throw new Error("Expected input Tensor to have rank " + expectedRank + ", " + ("but got rank " + input.shape.length + " that differs "));
      }
      ...
      if (!tf.util.arraysEqual(nonBatchedShape, expectedNonBatchShape)) {
        throw new Error("Expected input to have shape [null," + expectedNonBatchShape + "], " + ("but got shape [null," + nonBatchedShape + "]"));
      }
    };
  ...
  var TransferBrowserFftSpeechCommandRecognizer =
  /** @class */
  function (_super) {
    ...
    function TransferBrowserFftSpeechCommandRecognizer(name, parameters, baseModel) {
      ...
      tf.util.assert(name != null && typeof name === 'string' && name.length > 0, function () {
        return "The name of a transfer model must be a non-empty string, " + ("but got " + JSON.stringify(name));
      });
      _this.nonBatchInputShape = _this.baseModel.inputs[0].shape.slice(1);
      _this.words = null;
    ...
    TransferBrowserFftSpeechCommandRecognizer.prototype.collectExample = function (word, options) {
      return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
          tf.util.assert(!this.streaming, function () {
          ...
          tf.util.assert(word != null && typeof word === 'string' && word.length > 0, function () {
          ...
          if (options.durationSec != null) {
            ...
            numFramesPerSpectrogram = Math.ceil(options.durationSec / frameDurationSec_1);
          } else if (options.durationMultiplier != null) {
            tf.util.assert(options.durationMultiplier >= 1, function () {
              return "Expected duration multiplier to be >= 1, " + ("but got " + options.durationMultiplier);
            });
          ...
          if (options.snippetDurationSec != null) {
            tf.util.assert(options.snippetDurationSec > 0, function () {
              return "snippetDurationSec is expected to be > 0, but got " + ("" + options.snippetDurationSec);
            });
            tf.util.assert(options.onSnippet != null, function () {
              return "onSnippet must be provided if snippetDurationSec " + "is provided.";
            });
          }

          if (options.onSnippet != null) {
            tf.util.assert(options.snippetDurationSec != null, function () {
            ...
            var spectrogramCallback = function spectrogramCallback(freqData, timeData) {
              return __awaiter(_this, void 0, void 0, function () {
                ...
                return __generator(this, function (_p) {
                  switch (_p.label) {
                    case 0:
                      if (!(options.onSnippet == null)) return [3
                      /*break*/
                      , 7];
                    ...
                    case 1:
                      _c.spectrogram = (_d.data = _p.sent(), _d.frameSize = this.nonBatchInputShape[1], _d);
                      if (!options.includeRawAudio) return [3
                      /*break*/
                      , 3];
                      _f = {};
                      return [4
                    ...
                    case 2:
                      _e = (_f.data = _p.sent(), _f.sampleRateHz = this.audioDataExtractor.sampleRateHz, _f);
                      return [3
                      /*break*/
                      , 4];
                    ...
                    case 6:
                      _g.apply(void 0, [(_h.data = _p.sent(), _h.frameSize = this.nonBatchInputShape[1], _h)]);
                    ...
                    case 8:
                      ...
                      if (!(callbackCount++ === callbackCountTarget)) return [3
                      /*break*/
                      , 13];
                      return [4
                      /*yield*/
                    ...
                    case 10:
                      _m = (_o.data = _p.sent(), _o.sampleRateHz = this.audioDataExtractor.sampleRateHz, _o);
                      return [3
                      /*break*/
                      , 12];
                    ...
                    case 12:
                      _k.apply(_j, [(_l.rawAudio = _m, _l)]); // TODO(cais): Fix 1-tensor memory leak.
    ...
    TransferBrowserFftSpeechCommandRecognizer.prototype.countExamples = function () {
      if (this.dataset.empty()) {
        throw new Error("No examples have been collected for transfer-learning model " + ("named '" + this.name + "' yet."));
      }
    ...
    TransferBrowserFftSpeechCommandRecognizer.prototype.collectTransferDataAsTensors = function (windowHopRatio, augmentationOptions) {
      ...
      var out = this.dataset.getData(null, _assign({
        numFrames: numFrames,
        hopFrames: hopFrames
      }, augmentationOptions));
      return {
    ...
    TransferBrowserFftSpeechCommandRecognizer.prototype.collectTransferDataAsTfDataset = function (windowHopRatio, validationSplit, batchSize, augmentationOptions) {
      ...
      windowHopRatio = windowHopRatio || DEFAULT_WINDOW_HOP_RATIO;
      var hopFrames = Math.round(windowHopRatio * numFrames);
      return this.dataset.getData(null, _assign({
        numFrames: numFrames,
        hopFrames: hopFrames,
        getDataset: true,
      ...
      return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
          tf.util.assert(this.words != null && this.words.length > 0, function () {
            return "Cannot train transfer-learning model '" + _this.name + "' because no " + "transfer learning example has been collected.";
          });
          tf.util.assert(this.words.length > 1, function () {
            return "Cannot train transfer-learning model '" + _this.name + "' because only " + ("1 word label ('" + JSON.stringify(_this.words) + "') ") + "has been collected for transfer learning. Requires at least 2.";
          });

          if (config.fineTuningEpochs != null) {
            tf.util.assert(config.fineTuningEpochs >= 0 && Number.isInteger(config.fineTuningEpochs), function () {
              return "If specified, fineTuningEpochs must be a non-negative " + ("integer, but received " + config.fineTuningEpochs);
            });
          }
          ...
          if (this.dataset.durationMillis() > datasetDurationMillisThreshold) {
            console.log("Detected large dataset: total duration = " + (this.dataset.durationMillis() + " ms > ") + (datasetDurationMillisThreshold + " ms. ") + "Training transfer model using fitDataset() instead of fit()");
            return [2
            /*return*/
            , this.trainOnDataset(config)];
          } else {
            return [2
            /*return*/
            , this.trainOnTensors(config)];
          }
    ...
    TransferBrowserFftSpeechCommandRecognizer.prototype.trainOnDataset = function (config) {
      return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
          switch (_b.label) {
            case 0:
              tf.util.assert(config.epochs > 0, function () {
              ...
              _a = __read(this.collectTransferDataAsTfDataset(windowHopRatio, config.validationSplit, batchSize, {
                augmentByMixingNoiseRatio: config.augmentByMixingNoiseRatio
              }), 2), trainDataset = _a[0], valDataset = _a[1];
              t0 = tf.util.now();
              return [4
              /*yield*/
              , this.model.fitDataset(trainDataset, {
            ...
            case 1:
              ...
              if (!(config.fineTuningEpochs != null && config.fineTuningEpochs > 0)) return [3
              /*break*/
              , 3];
              t0_1 = tf.util.now();
              return [4
              /*yield*/
              , this.fineTuningUsingTfDatasets(config, trainDataset, valDataset)];

            case 2:
              fineTuningHistory = _b.sent();
              console.log("fitDataset() (fine-tuning) took " + ((tf.util.now() - t0_1).toFixed(2) + " ms"));
              return [2
              /*return*/
      ...
      return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
          switch (_b.label) {
            case 0:
              windowHopRatio = config.windowHopRatio || DEFAULT_WINDOW_HOP_RATIO;
              _a = this.collectTransferDataAsTensors(windowHopRatio, {
                augmentByMixingNoiseRatio: config.augmentByMixingNoiseRatio
              }), xs = _a.xs, ys = _a.ys;
            ...
            case 1:
              _b.trys.push([1,, 6, 7]); // TODO(cais): The balanced split may need to be pushed down to the
            ...
            case 2:
              ...
              return [4
              /*yield*/
              , this.fineTuningUsingTensors(config, trainXs, trainYs, valData)];

            case 3:
              fineTuningHistory = _b.sent();
              return [2
              /*return*/
              , [history_1, fineTuningHistory]];
      ...
      return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
          switch (_a.label) {
            ...
            case 1:
              fineTuningHistory = _a.sent(); // Set the trainable attribute of the fine-tuning layer to its
              // previous value.
    ...
    TransferBrowserFftSpeechCommandRecognizer.prototype.evaluate = function (config) {
      return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
          tf.util.assert(config.wordProbThresholds != null && config.wordProbThresholds.length > 0, function () {
            return "Received null or empty wordProbThresholds";
          });
          NOISE_CLASS_INDEX = 0;
          tf.util.assert(this.words[NOISE_CLASS_INDEX] === BACKGROUND_NOISE_TAG, function () {
          ...
          , tf.tidy(function () {
            ...
            var _a = _this.collectTransferDataAsTensors(config.windowHopRatio),
                xs = _a.xs,
                ys = _a.ys;

            var indices = ys.argMax(-1).dataSync();

            var probs = _this.model.predict(xs); // To calcaulte ROC, we collapse all word probabilites into a single
    ...
    TransferBrowserFftSpeechCommandRecognizer.prototype.createTransferModelFromBaseModel = function () {
      ...
      tf.util.assert(this.words != null, function () {
        return "No word example is available for tranfer-learning model of name " + _this.name;
        ...
        return __generator(this, function (_b) {
          switch (_b.label) {
            case 0:
              ...
              return [4
              /*yield*/
              , tfl.loadLayersModel(handlerOrURL)];
      ...
      return __generator(this, function (_a) {
        switch (_a.label) {
          case 0:
            ...
            return [4
            /*yield*/
            , tf.io.removeModel(getCanonicalSavePath(name))];
  ...
  function create(fftType, vocabulary, customModelArtifactsOrURL, customMetadataOrURL) {
    tf.util.assert(customModelArtifactsOrURL == null && customMetadataOrURL == null || customModelArtifactsOrURL != null && customMetadataOrURL != null, function () {
